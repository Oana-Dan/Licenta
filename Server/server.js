...
app.use(function (req, res, next) {
    res.header("Access-Control-Allow-Origin", "http://192.168.0.38:3000");
    res.header(
...
app.get('/users/:euser/shoppingcarts', async (req, res, next) => {
    try {
        const user = await Users.findByPk(req.params.euser);
        if (user) {
...
app.get('/daysofweeks/:dayofweekname/:dayofweekdate', async (req, res, next) => {
    try {
        const dayofweek = await DayOfWeeks.findOne({
            where: {
                nume: req.params.dayofweekname,
...
app.post('/daysofweeks/:dayofweekname/:dayofweekdate/dayorderedproducts', async (req, res, next) => {
    try {
        const dayofweek = await DayOfWeeks.findOne({
            where: {
                nume: req.params.dayofweekname,
        ...
        if (dayofweek) {
            ...
            dayorderedproduct.DayOfWeekNume = dayofweek.nume;
...
app.get('/daysofweeks/:dayofweekname/:dayofweekdate/dayorderedproducts/:dayorderedproductname', async (req, res, next) => {
    try {
        const dayofweek = await DayOfWeeks.findOne({
            where: {
                nume: req.params.dayofweekname,
...
app.put('/daysofweeks/:dayofweekname/:dayofweekdate/dayorderedproducts/:dayorderedproductname', async (req, res, next) => {
    try {
        const dayofweek = await DayOfWeeks.findOne({
            where: {
                nume: req.params.dayofweekname,
...
app.get('/tables/:tablenumber/datestables/:date', async (req, res, next) => {
    try {
        const table = await Tables.findByPk(req.params.tablenumber);
        if (table) {
            const datetable = await table.getDatesTables({
                where: {
...
app.post('/tables/:tablenumber/datestables/:date/hourstables', async (req, res, next) => {
    try {
        ...
        if (table) {
            ...
            console.log(datetable.data)
            if (datetable[0].data === req.params.date) {
                const hourtable = new HoursTables(req.body);
                hourtable.DatesTableData = datetable[0].data;
...
server.listen(8000, '127.0.0.1', function () {
    server.close(function () {
        server.listen(8080, '192.168.0.38')
    })
})
